      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-zinc-400 text-sm">Download your generated PDF biomechanics reports.</p>
          </div>
          <div className="text-right">
            <span className="text-2xl font-black text-white">{reports.length}</span>
            <p className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">Available</p>
          </div>
        </div>
        ...
        {reports.length === 0 ? (
          ...
          <div className="grid gap-4">
            {reports.map((r, i) => {
              ...
              return (
                <div key={id} className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 rounded-2xl border border-zinc-800 bg-zinc-950/60 hover:bg-zinc-900/40 transition">
                  
                  <div className="flex items-center gap-6">
                    <div className="h-14 w-14 rounded-xl bg-zinc-900 flex items-center justify-center border border-zinc-800">
                      <FileText className="h-6 w-6 text-zinc-400" />
                    </div>
                    <div>
                      <h3 className="text-sm font-bold text-white mb-1 tracking-wide">Biomechanics Report #{reports.length - i}</h3>
                      <div className="flex items-center gap-4 text-xs text-zinc-500 font-semibold">
                        ...
                        <span className="flex items-center gap-1.5"><Activity className="h-3 w-3" /> Score: {r.scores?.performanceScore || 'N/A'}</span>
                  ...
                  <div className="flex items-center gap-3">
                    {r.reportReady ? (
                      <a
                        ...
                        className="flex items-center gap-2 bg-white text-black px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-zinc-200 transition"
                      >
                        <Download className="h-4 w-4" />
                        Download PDF
                      </a>
                    ) : (
                      <button disabled className="flex items-center gap-2 bg-zinc-900 text-zinc-500 px-5 py-2.5 rounded-xl text-sm font-bold cursor-not-allowed border border-zinc-800">
                        Pending Generation
                      </button>
                    )}
                  </div>
