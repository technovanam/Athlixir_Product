...
import { SYSTEM_PROMPT, buildAnalysisPrompt, buildAthleteChatPrompt, buildEvolutionInsightsPrompt } from './prompts/insights-prompts';
...
export class AiInsightsService {
   ...
   */
  async generateIntelligence(analysisId: string, userId: string, rawData: any): Promise<any> {
    this.logger.log(`Running AI Sports Intelligence Orchestrator for analysis: ${analysisId}`);

    const metrics = rawData.metrics || {};
    const scores = rawData.scores || {};
    const injuryRisks = rawData.injuryRisks || [];
    const benchmarks = rawData.benchmarks || { profileLabel: 'State Sprint Benchmark', levels: { cadence: 'State', gct: 'State', strideLength: 'State' } };
    ...
    try {
      ...
      const validationResult = this.metricValidatorService.validateMetrics(metrics);
      const pythonFlags = Array.isArray(rawData?.metricFlags) ? rawData.metricFlags : [];
      const metricFlags = Array.from(new Set([...validationResult.metricFlags, ...pythonFlags]));
      ...
      const userPrompt = buildAnalysisPrompt(promptInput, metricFlags, confidence);
      ...
      try {
        rawResponse = await this.claudeService.generateCompletion(systemPrompt, userPrompt);
        this.logger.log('AI intelligence generated successfully using Anthropic Claude.');
      } catch (claudeErr: any) {
        this.logger.warn(`Claude completion failed: ${claudeErr.message}. Falling back to Google Gemini.`);
        try {
          usedProvider = 'Gemini';
          rawResponse = await this.geminiService.generateCompletion(systemPrompt, userPrompt);
          this.logger.log('AI intelligence generated successfully using Google Gemini.');
        } catch (geminiErr: any) {
          this.logger.error(`Gemini completion failed as well: ${geminiErr.message}. Triggering local high-fidelity simulator.`);
          rawResponse = JSON.stringify(this.aiParserService.generateSafetyFallback(metrics));
          usedProvider = 'LocalSimulator';
        }
      }

      // 4. Parse & Validate structured JSON output
      const parsedAiResults = this.aiParserService.parseAndValidateResponse(rawResponse, metrics, rawData);
      ...
      const reportHtml = generateSportsScienceReportHtml(analysisId, athleteContext.name, {
        athlete: athleteContext,
        metrics,
        scores,
        benchmarks,
      ...
      this.logger.log(`Extended database schema in Firestore for ${analysisId} successfully.`);

      // Store in sub-collections for progressive dashboard analytics if necessary
      await db.collection('reports').doc(analysisId).set({
        analysisId,
        userId,
      ...
      this.logger.error(`Failed during AI intelligence orchestration: ${err.message}. Saving emergency fallback.`);
      ...
      const reportHtml = generateSportsScienceReportHtml(analysisId, 'Athlete', {
        athlete: { name: 'Athlete', ageGroup: 'u18', gender: 'male', event: '100m' },
        metrics,
        scores,
        benchmarks,
        injuryRisk: rawData.injuryRisk,
   ...
   */
  async handleAthleteChat(analysisId: string, userId: string, message: string): Promise<string> {
    this.logger.log(`User ${userId} chatting with assistant regarding analysis ${analysisId}`);

    try {
      const db = this.firebaseService.firestore;
      
      // 1. Fetch current analysis
      const analysisDoc = await db.collection(this.collectionName).doc(analysisId).get();
      ...
      const profileDoc = await db.collection(this.profileCollectionName).doc(userId).get();
      const profile = profileDoc.exists ? (profileDoc.data() as any) : { gender: 'male', primary_event: '100m' };
      ...
      // 4. Construct prompts
      const prompts = buildAthleteChatPrompt(history, { ...profile, name }, latestAnalysis, message);

      // 5. Query LLM
      let response = '';
      try {
        response = await this.claudeService.generateCompletion(prompts.system, prompts.user);
      } catch (err: any) {
        this.logger.warn(`Claude chat failed: ${err.message}. Trying Gemini...`);
        try {
          response = await this.geminiService.generateCompletion(prompts.system, prompts.user, false);
        } catch (geminiErr: any) {
          this.logger.error(`Gemini chat failed as well: ${geminiErr.message}. Resolving with local expert guidelines.`);
   ...
   */
  async generateEvolutionInsights(userId: string, evolutionData: any): Promise<any> {
    this.logger.log(`Generating evolution/progress insights for user: ${userId}`);

    try {
      const athleteContext = await this.getAthleteContext(userId);
      const prompt = buildEvolutionInsightsPrompt(athleteContext as any, evolutionData);

      let rawResponse = '';
      try {
        rawResponse = await this.claudeService.generateCompletion(SYSTEM_PROMPT, prompt);
      } catch (err: any) {
        this.logger.warn(`Claude progress insights failed: ${err.message}. Trying Gemini...`);
        try {
          rawResponse = await this.geminiService.generateCompletion(SYSTEM_PROMPT, prompt, true);
        } catch (geminiErr: any) {
          this.logger.error(`Gemini progress insights failed: ${geminiErr.message}. Falling back to default.`);
      ...
      this.logger.error(`Failed to parse evolution insights JSON: ${err.message}`);
      return this.generateDefaultEvolutionInsights(evolutionData);
    }
  }
  ...
  private generateDefaultEvolutionInsights(evolution: any): any {
    ...
    if (evolution.cadence_growth) {
      highlights.push(`Cadence growth of ${evolution.cadence_growth} over the progression window.`);
    }
    if (evolution.gct_reduction && parseInt(evolution.gct_reduction) !== 0) {
      highlights.push(`Ground Contact Time reduced by ${Math.abs(parseInt(evolution.gct_reduction))}ms.`);
    }
    if (highlights.length === 0) {
      highlights.push(`Completed ${evolution.sessionCount} scan sessions to establish baseline mechanics.`);
    }
    ...
    return {
      ...
      progressCommentary: 'Maintain consistent stride-frequency intervals to further stabilize cadence output.',
      highlights,
    };
  }
  ...
  private async getAthleteContext(userId: string) {
    try {
      ...
      if (!doc.exists) {
        return { name: 'Athlete', ageGroup: 'u18', gender: 'male', event: '100m' };
      }
      ...
      const ageGroup = d.dob ? (this.calculateAge(d.dob) < 18 ? 'u18' : 'open') : 'u18';

      return {
        name: d.firstName && d.lastName ? `${d.firstName} ${d.lastName}` : d.username || 'Athlete',
        ageGroup,
        gender,
        event,
  ...
  private generateLocalExpertChatReply(message: string, analysis: any): string {
    ...
    if (query.includes('gct') || query.includes('ground contact') || query.includes('contact')) {
